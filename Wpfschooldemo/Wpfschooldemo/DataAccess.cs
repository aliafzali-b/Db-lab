            ...
            using (IDbConnection connection = new System.Data.SqlClient.SqlConnection(Helper.CnnVal("dbschool")))
            {
                try
                {
                    connection.Execute(
                        ...
                        $"CREATE TABLE Class(ClassID int NOT NULL,_Name nvarchar(255) NOT NULL,MajorID int NULL,BranchNumber int NULL,_Year nvarchar(255) NULL,ChairNum int NULL,PRIMARY KEY(ClassID),FOREIGN KEY(MajorID) REFERENCES Major(MajorID));" +
                        $"CREATE TABLE Student(StuID int NOT NULL,Username varchar(255) NOT NULL UNIQUE,__Password varchar(255) NOT NULL,_Name nvarchar(255) NOT NULL,LastName nvarchar(255) NOT NULL,FatherName nvarchar(255) NOT NULL,Phone bigint NOT NULL,Addres nvarchar(255) NULL,Info nvarchar(255) NULL,ClassID int NULL,MajorID int NULL,Gender char(1) NULL,PRIMARY KEY CLUSTERED(StuID),CONSTRAINT CheckStudent check(Gender = 'M' OR Gender = 'F'),FOREIGN KEY(ClassID) REFERENCES Class(ClassID),FOREIGN KEY(MajorID) REFERENCES Major(MajorID));" +
                        $"CREATE TABLE Exam(ExamID int NOT NULL,CourseID int NOT NULL,ClassID int NOT NULL,_Date datetime NOT NULL,Ratio float(10) NOT NULL,Info nvarchar(255) NULL,Status char(1) NULL,PRIMARY KEY(ExamID),FOREIGN KEY(ClassID) REFERENCES Class(ClassID),FOREIGN KEY(CourseID) REFERENCES Courses(CourseID));" +
                        $"CREATE TABLE Grade(ExamID int NOT NULL,StuID int NOT NULL,Grade int NULL,PRIMARY KEY(ExamID),FOREIGN KEY(ExamID) REFERENCES Exam(ExamID),FOREIGN KEY(StuID) REFERENCES Student(StuID));" +
