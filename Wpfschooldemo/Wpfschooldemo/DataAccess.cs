...
namespace Wpfschooldemo
{
    class DataAccess
    {
        ...
        public void insertIntoStudentsTable(string username, string password, string name, string fathername, long phone, string major, int classid)
        {
            using (IDbConnection connection = new System.Data.SqlClient.SqlConnection(Helper.CnnVal("dbschool")))
            {
                try
                {
                    ...
                    if (!String.IsNullOrWhiteSpace(major))
                        connection.Execute($"insert into Students values({range + 1},'{username}','{password}',N'{name}',N'{fathername}',{phone},N'{major}',{classid})");
                    else
                        connection.Execute($"insert into Students values({range + 1},'{username}','{password}',N'{name}',N'{fathername}',{phone}, NULL ,{classid})");
                    //MessageBox.Show("اضافه گردید", "Added", MessageBoxButton.OK, MessageBoxImage.Information);
                }
                catch { MessageBox.Show("cant"); }
            }
        ...
        public List<Students> SearchStudents(string type, string value)
        {
            using (IDbConnection connection = new System.Data.SqlClient.SqlConnection(Helper.CnnVal("dbschool")))
            {
                if (type != "_classname" && type != "_name")
                {
                    var output = connection.Query<Students>($"SELECT students._id, students._username, students._password, students._name, students._fathername, students._phonenumber, students._major,classes._id as _classid, classes._name as _classname FROM students INNER JOIN classes ON students._classid = classes._id Where {type} like N'%{value}%' order by _id").ToList();
                    return output;
                }
                else if (type != "_name")
                {
                    var output = connection.Query<Students>($"SELECT students._id, students._username, students._password, students._name, students._fathername, students._phonenumber, students._major,classes._id as _classid, classes._name as _classname FROM students INNER JOIN classes ON students._classid = classes._id Where classes._name like N'%{value}%' order by _id").ToList();
                    return output;
                }else
                {
                    var output = connection.Query<Students>($"SELECT students._id, students._username, students._password, students._name, students._fathername, students._phonenumber, students._major,classes._id as _classid, classes._name as _classname FROM students INNER JOIN classes ON students._classid = classes._id Where Students._name like N'%{value}%' order by _id").ToList();
                    return output;
                }
            }
        }
        ...
        public void deleteStudentByUserName(string username)
        {
            using (IDbConnection connection = new System.Data.SqlClient.SqlConnection(Helper.CnnVal("dbschool")))
            {
                try
                {
                    ...
                    for (int i = deleteID; i < range; i++)
                    {
                        connection.Execute($"UPDATE Students SET _id = {i} WHERE _id = {i + 1};");
                    }
                    MessageBox.Show("حذف گردید", "Deleted", MessageBoxButton.OK, MessageBoxImage.Information);
                }
                catch { MessageBox.Show("db error"); }
        ...
        public int Create_All_Tables()
        {
            using (IDbConnection connection = new System.Data.SqlClient.SqlConnection(Helper.CnnVal("dbschool")))
            {
                try
                {
                    connection.Execute($"CREATE TABLE Major (MajorID int NOT NULL,_Name nvarchar(255) NOT NULL,MCode int NULL,PRIMARY KEY(MajorID));CREATE TABLE Courses(CourseID int NOT NULL,_Name nvarchar(255) NOT NULL,CCode int NULL, Unit int NOT NULL,PRIMARY KEY(CourseID));CREATE TABLE Class(ClassID int NOT NULL,_Name nvarchar(255) NOT NULL,MajorID int NULL,BranchNum int NULL,_Year nvarchar(255) NULL,ChairNum int NULL,PRIMARY KEY(ClassID),FOREIGN KEY(MajorID) REFERENCES Major(MajorID),);CREATE TABLE Student(StuID int NOT NULL,_Name nvarchar(255) NOT NULL,FamilyName nvarchar(255) NOT NULL,Addres nvarchar(255) NULL,ClassID int NULL,Username varchar(255) NOT NULL UNIQUE,__Password varchar(255) NOT NULL,Phone bigint NOT NULL,FatherName nvarchar(255) NOT NULL,MajorID int NULL,Info nvarchar(255) NULL,Gender char(1) NULL,PRIMARY KEY CLUSTERED(StuID),CONSTRAINT CheckStudent check(Gender = 'M' OR Gender = 'F'),FOREIGN KEY(ClassID) REFERENCES Class(ClassID),FOREIGN KEY(MajorID) REFERENCES Major(MajorID),);CREATE TABLE Exam(ExamID int IDENTITY NOT NULL,ClassID int NOT NULL,_Date datetime NOT NULL,CourseID int NOT NULL,Ratio float(10) NOT NULL,Info nvarchar(255) NULL,Status char(1) NULL,PRIMARY KEY(ExamID),FOREIGN KEY(ClassID) REFERENCES Class(ClassID),FOREIGN KEY(CourseID) REFERENCES Courses(CourseID),);CREATE TABLE Grade(ExamID int NOT NULL,StuID int NOT NULL,Grade int NULL,PRIMARY KEY(ExamID),FOREIGN KEY(ExamID) REFERENCES Exam(ExamID),FOREIGN KEY(StuID) REFERENCES Student(StuID),);CREATE TABLE Teacher(TeacherID int NOT NULL,_Name nvarchar(255) NOT NULL,Family nvarchar(255) NOT NULL,Expert nvarchar(255) NULL,UserName varchar(255) NOT NULL UNIQUE,_Password varchar(255) NOT NULL,Email int NOT NULL,Gender char(1) NULL,PRIMARY KEY(TeacherID),CONSTRAINT CheckTeacher check(Gender = 'M' OR Gender = 'F'),);CREATE TABLE Manager(ManagerID int NOT NULL,_Name nvarchar(255) NOT NULL,Family nvarchar(255) NOT NULL,Adress nvarchar(255) NULL,Username varchar(255) NOT NULL UNIQUE,_Password varchar(255) NOT NULL,Email nvarchar(255) NOT NULL,RememberMe int NOT NULL,PRIMARY KEY(ManagerID));CREATE TABLE CoCoT(CourseID int NOT NULL,ClassID int NOT NULL,TeacherID int NOT NULL);");
